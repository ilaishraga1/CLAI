...
class MultiTaxiEnv(ParallelEnv):
    ...
    def __step_from_state(self, state, joint_action):
        ...
        for taxi_name, action in joint_action.items():
            ...
            if action in {a.value for a in MOVE_ACTIONS}:
                self.__move_taxi(taxi, action, infos, rewards, new_state.passengers)
    ...
    def __check_new_loc_inside_map(self, new_loc):
        return new_loc[0] >= 0 and new_loc[0] < self.domain_map.map_height \
            and new_loc[1] >= 0 and new_loc[1] < self.domain_map.map_width
         
    def __check_closer_to_passenger(self, taxi, new_loc, passengers)

    def __move_taxi(self, taxi, move_action, infos, rewards, passengers):
        self.__add_reward(taxi.name, infos, rewards, Event.MOVE)
        new_loc = taxi.move(move_action, simulation=True)
        if not self.__check_new_loc_inside_map(new_loc):
            self.__add_reward(taxi.name, infos, rewards, Event.MOVE, -3)
